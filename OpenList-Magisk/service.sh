...
# 查找可用的 busybox 路径，兼容 Magisk / KSU / APatch（与 action.sh 统一）
find_busybox() {
    for path in \
...
log() {
    # 日志轮转（限制日志文件大小 1MB，用 wc -c 兼容所有 Android 版本）
...
get_lan_ip() {
    ...
    while [ $RETRY_COUNT -lt $MAX_RETRY ]; do
        ...
        if [ -n "$ip_address" ] && [ "$ip_address" != "无法获取IP" ]; then
            log "成功获取 IP: $ip_address (尝试次数: $((RETRY_COUNT + 1)))"
...
get_port() {
    ...
    while [ $RETRY_COUNT -lt $MAX_RETRY ]; do
        ...
        if [ -n "$port" ]; then
            log "成功获取端口: $port (尝试次数: $((RETRY_COUNT + 1)))"
            echo "$port" > "$TEMP_PORT_FILE"
            return 0
        fi
        log "未获取到端口 (PID: $pid, 尝试 $((RETRY_COUNT + 1))/$MAX_RETRY)，1秒后重试"
        sleep 1
        RETRY_COUNT=$((RETRY_COUNT + 1))
    done

    log "错误: 获取端口超时 (PID: $pid)"
    echo "" > "$TEMP_PORT_FILE"
}
...
update_module_prop_running() {
    ...
    while [ $ELAPSED -lt $MAX_WAIT ]; do
        if [ -f "$TEMP_IP_FILE" ] && [ -f "$TEMP_PORT_FILE" ]; then
            log "IP 和端口获取完成 (耗时: $ELAPSED 秒)"
            break
        fi
        sleep 1
        ELAPSED=$((ELAPSED + 1))
    ...
    if [ -n "$port" ] && [ "$CURRENT_IP" != "无法获取IP" ]; then
        ...
        NEW_DESC="description=【运行中】无法检测地址（IP: $CURRENT_IP, 端口: $port，PID:$pid），请检查日志 $LOG_FILE | 数据目录：${DATA_DIR} | 点击▲操作关闭程序${PASSWORD_TEXT}"
    fi
    ...
    if [ ! -w "$MODULE_PROP_FILE" ]; then
        ...
        chmod 644 "$MODULE_PROP_FILE" 2>/dev/null || log "错误: 无法设置权限"
    fi
...
if [ ! -w "$DATA_DIR" ]; then
    log "警告: 数据目录不可写，尝试修复权限"
    chmod 755 "$DATA_DIR" 2>/dev/null || {
        log "错误: 无法设置数据目录权限"
        exit 1
    }
fi
log "已验证数据目录：$DATA_DIR"
...
# 等待 Android 启动完成，每 10 秒记录一次，避免日志刷屏
ELAPSED=0
MAX_WAIT=60
while [ $ELAPSED -lt $MAX_WAIT ]; do
    if [ "$(getprop sys.boot_completed)" = "1" ]; then
        log "Android 系统启动完成（等待了 $ELAPSED 秒）"
        break
    fi
    [ $((ELAPSED % 10)) -eq 0 ] && log "等待 Android 系统启动... ($ELAPSED/$MAX_WAIT 秒)"
    sleep 1
    ELAPSED=$((ELAPSED + 1))
done
